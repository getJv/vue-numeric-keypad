...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const pad = ref<string>('000');
    const formatedAmount = computed((): string => {
      const arr = [...pad.value];
      arr.splice(arr.length - 2, 0, props.config.decimalSignal);
      return arr.join('');
    });
    const amount = computed((): number => Number(formatedAmount.value.replace(',', '.')));

    const notifyEvents = (pressedButton: string, shouldEmitConfirm: boolean) => {
      emit('pad-keyup', pressedButton);
      emit('amount', amount);
      emit('formatedAmount', formatedAmount);
